...
ig.pred <- ig%>%
 ...
 summarize(
  ...
  left_join(ig%>%dplyr::select(j.day,date))
...
vc.pred <- vc%>%
 ...
 summarize(
  ...
  left_join(vc%>%dplyr::select(j.day,date))
...
cc.pred <- cc%>%
 ...
 summarize(
  ...
  left_join(cc%>%dplyr::select(j.day,date))
...
ce.pred <- ce%>%
 ...
 summarize(
  ...
  left_join(ce%>%dplyr::select(j.day,date))
...
vc.arr.weath <- vc.arrive.date%>%
  left_join(weather.d)%>%
  left_join(vc%>%dplyr::select(year,date,j.day))
head(vc.arr.weath)
